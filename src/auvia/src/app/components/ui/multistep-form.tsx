...
const steps: Step[] = [
  ...
  { id: 3, label: "Phone Number", field: "phone", placeholder: "+1 555 123 4567", type: "tel" },
  { id: 4, label: "Email", field: "email", placeholder: "you@yourclinic.com", type: "email" },
];
...
export function MultiStepForm({ onClose }: MultiStepFormProps) {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      setSubmitError("Couldn’t send right now. Please try again.");
    } finally {
        ...
        <div className="relative overflow-hidden rounded-2xl border border-border/50 bg-gradient-to-br from-background via-background to-muted/20 p-12">
          ...
          <div className="relative flex flex-col items-center gap-4 animate-in fade-in zoom-in-95 duration-700">
            ...
            <div className="space-y-1 text-center">
              ...
              <p className="text-xs text-muted-foreground/70 pt-1">Our team will reach out to {formData.email} / {formData.phone} shortly.</p>
            </div>
            {onClose && (
        ...
        >
          <span className="flex items-center justify-center gap-2 font-medium">
            {currentStep === steps.length - 1 ? (
              <>{isSubmitting ? "Sending..." : "Send"} <Send className="h-4 w-4 transition-transform group-hover:translate-x-0.5 duration-300" strokeWidth={2} /></>
            ) : (
