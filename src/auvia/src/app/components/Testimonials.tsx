...
//   const testimonials = [
//     {
//       quote: "Since using NexovAI, our clinic never misses patient calls. The AI receptionist handles everything seamlessly.",
...
//             See what doctors and clinic administrators say about NexovAI
//           </p>
//         </motion.div>
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((testimonial, index) => (
            <motion.div
              ...
              className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-8 shadow-sm transition-all hover:shadow-lg"
            >
              <Quote className="mb-4 size-8 text-[#84aca6]" />
              <p className="mb-6 text-lg leading-relaxed text-slate-700">{testimonial.quote}</p>
              <div className="flex items-center gap-4">
                <div className="size-12 rounded-full bg-[#84aca6]"></div>
                <div>
                  <p className="font-bold text-slate-900">{testimonial.author}</p>
                  <p className="text-sm text-slate-600">{testimonial.role}</p>
                  <p className="text-sm text-[#84aca6]">{testimonial.clinic}</p>
                </div>
              </div>
            </motion.div>
          ))}
